...
public abstract class GameObject {
    ...
    public void shoot(GameObject receivingObject, String message) {
        if (receivingObject == this) {
            game.getGameOutput().appendln(IOUtils.prefixWithAsterisk("You can't shoot an object with itself."));
        }
        else {
            if (message != null) {
    ...
    public void takeDamage(int amount) {
        if (amount == 0) {
            game.getGameOutput().appendln(IOUtils.prefixWithAsterisk(IOUtils.capitalizeFirstLetter(IOUtils.getNickNameOrNameWithArticle(this)) + " takes no damage."));
            return;
        }

        if (this.health - amount > 0) {
            this.health -= amount;
            game.getGameOutput().appendln(IOUtils.prefixWithAsterisk(IOUtils.capitalizeFirstLetter(IOUtils.getNickNameOrNameWithArticle(this)) + " takes damage."));
        }
        else {
            destroy(true);
        }
    ...
    public void destroy(boolean displayDestroyString) {
        ...
        if (displayDestroyString) {
            game.getGameOutput().appendln(IOUtils.prefixWithAsterisk(IOUtils.capitalizeFirstLetter(IOUtils.getNickNameOrNameWithArticle(this)) + " has been destroyed. " + IOUtils.getRandomDestroyString()));
        }
    }
    ...
    public boolean receiveInsertInto(GameObject actingObject) {
        game.getGameOutput().appendln(IOUtils.prefixWithAsterisk("Items cannot be placed inside " + IOUtils.getNickNameOrNameWithArticle(this)));
        return false;
    }
    ...
    public void removeFrom(GameObject receivingObject) {
        if (receivingObject instanceof Container) {
            ...
            game.getGameOutput().appendln(IOUtils.prefixWithAsterisk("Nothing can be taken from inside " + IOUtils.capitalizeFirstLetter(IOUtils.getNickNameOrNameWithArticle(receivingObject)) + ". Everything in there is still a part of " + IOUtils.getNickNameOrNameWithArticle(receivingObject) + "."));
        }
    }
